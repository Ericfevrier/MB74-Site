...
export function mountAdmin(app) {
  ...
  const CUSTOM_FIELDS = ['slug', 'title', 'blocks', 'seo', 'status', 'sort_order'];
  const rowToCustomPage = (r, admin = false) => {
    const p = {
      slug: r.slug,
      title: r.title || '',
  ...
  const customPageToRow = (p) => ({
    slug: String(p.slug || '').trim().toLowerCase(),
    title: String(p.title || '').trim(),
    blocks: JSON.stringify(Array.isArray(p.blocks) ? p.blocks : []),
    seo: p.seo && Object.keys(p.seo).length ? JSON.stringify(p.seo) : null,
    status: p.status === 'draft' ? 'draft' : 'published',
    sort_order: Number.isFinite(Number(p.sortOrder)) ? Number(p.sortOrder) : 0,
  });

  // Lecture publique d'une page libre (aperçu brouillon si ?preview=1 + admin connecté).
  app.get('/api/custom-pages/:slug', async (req, res) => {
    if (!dbConfigured()) return needDb(res);
    const preview = req.query.preview && currentAdmin(req);
    try {
      const where = preview ? 'slug = ?' : "slug = ? AND status = 'published'";
      const rows = await query(`SELECT * FROM custom_pages WHERE ${where} LIMIT 1`, [req.params.slug]);
      if (!rows.length) return res.status(404).json({ ok: false, error: 'Page introuvable.' });
      res.json({ page: rowToCustomPage(rows[0]) });
    } catch (e) {
      console.error('GET /api/custom-pages/:slug', e.message);
      res.status(500).json({ ok: false, error: 'Erreur base de données.' });
    }
  });

  app.get('/api/admin/custom-pages', requireAuth, async (_req, res) => {
    if (!dbConfigured()) return needDb(res);
    try {
      const rows = await query('SELECT id, slug, title, status, sort_order FROM custom_pages ORDER BY sort_order ASC, id DESC');
      res.json({ pages: rows });
    } catch (e) {
      console.error('GET /api/admin/custom-pages', e.message);
      res.status(500).json({ ok: false, error: 'Erreur base de données.' });
    }
  });

  app.get('/api/admin/custom-pages/:id', requireAuth, async (req, res) => {
    if (!dbConfigured()) return needDb(res);
    try {
      const rows = await query('SELECT * FROM custom_pages WHERE id = ? LIMIT 1', [Number(req.params.id)]);
      if (!rows.length) return res.status(404).json({ ok: false, error: 'Introuvable.' });
      res.json({ page: rowToCustomPage(rows[0], true) });
    } catch (e) {
      console.error('GET /api/admin/custom-pages/:id', e.message);
      res.status(500).json({ ok: false, error: 'Erreur base de données.' });
    }
  });

  const RESERVED_SLUGS = new Set([
    'bateaux', 'services', 'contact', 'blog', 'admin', 'marque', 'la-team', 'shop',
    'mentions-legales', 'politique-de-confidentialite', 'cgv-pro', 'hivernage-stockage-bateau',
    'depannage', 'sellerie', 'entretien-reparation', 'transport', 'remorques', 'nautique', 'mastercraft',
  ]);

  app.post('/api/admin/custom-pages', requireAuth, async (req, res) => {
    if (!dbConfigured()) return needDb(res);
    const row = customPageToRow(req.body || {});
    if (!row.slug || !row.title) return res.status(400).json({ ok: false, error: 'Titre et URL requis.' });
    if (!SLUG_RE.test(row.slug)) return badSlug(res);
    if (RESERVED_SLUGS.has(row.slug)) return res.status(409).json({ ok: false, error: 'Cette URL est réservée à une page du site.' });
    try {
      const cols = CUSTOM_FIELDS.join(', ');
      const ph = CUSTOM_FIELDS.map(() => '?').join(', ');
      const r = await query(`INSERT INTO custom_pages (${cols}) VALUES (${ph})`, CUSTOM_FIELDS.map((c) => row[c]));
      res.json({ ok: true, id: r.insertId });
    } catch (e) {
      if (e.code === 'ER_DUP_ENTRY') return res.status(409).json({ ok: false, error: 'Cette URL existe déjà.' });
      console.error('POST /api/admin/custom-pages', e.message);
      res.status(500).json({ ok: false, error: 'Erreur base de données.' });
    }
  });

  app.put('/api/admin/custom-pages/:id', requireAuth, async (req, res) => {
    if (!dbConfigured()) return needDb(res);
    const id = Number(req.params.id);
    const row = customPageToRow(req.body || {});
    if (!row.slug || !row.title) return res.status(400).json({ ok: false, error: 'Titre et URL requis.' });
    ...
    if (RESERVED_SLUGS.has(row.slug)) return res.status(409).json({ ok: false, error: 'Cette URL est réservée à une page du site.' });
    try {
      const set = CUSTOM_FIELDS.map((c) => `${c} = ?`).join(', ');
      const r = await query(`UPDATE custom_pages SET ${set} WHERE id = ?`, [...CUSTOM_FIELDS.map((c) => row[c]), id]);
      if (!r.affectedRows) return res.status(404).json({ ok: false, error: 'Introuvable.' });
      res.json({ ok: true });
    } catch (e) {
      if (e.code === 'ER_DUP_ENTRY') return res.status(409).json({ ok: false, error: 'Cette URL existe déjà.' });
      console.error('PUT /api/admin/custom-pages', e.message);
      res.status(500).json({ ok: false, error: 'Erreur base de données.' });
    }
  });

  app.delete('/api/admin/custom-pages/:id', requireAuth, async (req, res) => {
    if (!dbConfigured()) return needDb(res);
    try {
      const r = await query('DELETE FROM custom_pages WHERE id = ?', [Number(req.params.id)]);
      if (!r.affectedRows) return res.status(404).json({ ok: false, error: 'Introuvable.' });
      res.json({ ok: true });
    } catch (e) {
      console.error('DELETE /api/admin/custom-pages', e.message);
      res.status(500).json({ ok: false, error: 'Erreur base de données.' });
    }
  });
