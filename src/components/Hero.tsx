  ...
  return (
    <section className="relative flex-shrink-0 h-[500px] md:h-[600px] lg:h-[700px] overflow-hidden">
        ...
        <div className="w-full px-4 md:px-12">
          <div className="max-w-3xl mt-6 sm:mt-10 md:mt-[75px] pl-[5px]">
            <div className="flex items-center space-x-3 text-brand-cyan mb-3">
              ...
              <span className="uppercase tracking-[0.15em] sm:tracking-[0.3em] font-bold text-[11px] sm:text-[15px]">{t('hero.eyebrow')}</span>
            </div>

            <h1 className="text-white font-bold uppercase text-[clamp(1.75rem,5vw,2.75rem)] leading-[1.1] mb-5 tracking-tight">
              <span className="block">{t('hero.title1')}</span>
              <span className="text-brand-cyan block">{t('hero.title2')}</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 pt-6">
              <Link to={t('hero.cta1.url')} aria-label={t('hero.cta1.label')} className="inline-flex items-center justify-center bg-brand-cyan text-brand-dark px-6 sm:px-10 py-4 font-bold uppercase text-sm hover:translate-y-[-2px] hover:shadow-2xl hover:shadow-brand-cyan/40 transition-all duration-300 rounded-2xl min-h-[44px]">
                {t('hero.cta1.label')}
              </Link>
              <Link to={t('hero.cta2.url')} aria-label={t('hero.cta2.label')} className="inline-flex items-center justify-center bg-white/10 backdrop-blur-md border border-white/20 text-white px-6 sm:px-10 py-4 font-bold uppercase text-sm hover:bg-white/20 transition-all duration-300 rounded-2xl min-h-[44px]">
                {t('hero.cta2.label')}
              </Link>
            </div>
          </div>
      ...
      <div className="absolute top-10 right-4 md:right-12 hidden lg:flex z-20">
          <div className="bg-white/10 backdrop-blur-md px-6 py-3.5 rounded-2xl border border-white/20 flex items-center gap-3 shadow-2xl">
            <span className="text-white text-[16px] font-bold tracking-widest uppercase">{t('hero.badgeRight')}</span>
            <div className="flex items-center border-l border-white/20 pl-3">
              <img 
